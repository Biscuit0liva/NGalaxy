                           ...
                           const int     offset)
{
    ...
    // tile en memoria local
    __local float4 shPosition[BSIZE];
    ...
    for (int t = 0; t < loop; t++) {
        // cada hilo carga una posicion a local
        shPosition[lid] = pdata[start + t * BSIZE + lid];
        barrier(CLK_LOCAL_MEM_FENCE);

        acc = tile_calculation(myPosition, acc, shPosition);
