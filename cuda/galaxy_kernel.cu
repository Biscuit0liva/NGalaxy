...
__global__ void galaxyKernel(float4* pos, float4* pdata, int N, float step, int apprx, int offset) {
    ...
    for (int i = 0; i < loop; i++) {
        int idx = threadIdx.x;
        shPosition[idx] = pdata[start + i * BSIZE + idx];
        __syncthreads();
...
extern "C" void cudaComputeGalaxy(void* dptr, float4* pdata, int N, float step, int apprx, int offset) {
    float4* out = reinterpret_cast<float4*>(dptr);
    int blockSize = BSIZE;
    int numBlocks = (N + blockSize - 1) / blockSize;
    galaxyKernel<<<numBlocks, blockSize, BSIZE * sizeof(float4)>>>(out, pdata, N, step, apprx, offset);
}
